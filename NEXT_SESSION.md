...
### "Convergent, unconfirmed signal" report caveat — DONE (2026-07-03, commits `8ce8636`, `7675c80`)

Implements the report caveat proposed alongside the grounding-tier tagging brief: when a principle's static grounding tag (`artifact-principles.json`'s `speculative`/`plausible-untested` `inferenceTier`) combines with a `stable` runtime classification from Step B (see item below) on a specific object, the analysis gets a caveat explaining that cross-model agreement rules out one model inventing the claim but doesn't confirm the claim is correct — the principle has no dedicated training data confirming what the models are actually detecting.

**Deterministic, not LLM-authored:** `buildVerificationCaveats()` in `analyze.ts` generates this in code from the object's stored `fingerprint_synthesis_classification` and `ARTIFACT_GROUNDING` (new lookup in `principles.ts`, derived from `artifact-principles.json`'s `grounding` field). Appended to `pass2TextForDisplay` — a separate variable from the clean `pass2Text` that still feeds Pass 3/extraction/vector-scoring/contradiction-detection unchanged — so the caveat can't be softened or dropped by the model and never contaminates downstream passes.

**Two problems found via live testing on object 35, both fixed same day:**
1. **Volume** — every one of the 15 artifact principles is tagged `speculative` or `plausible-untested` (none `verifiable`, per Alan's conservative call on Symmetry/Proportion during the grounding-tier work), so *every* `stable`-tier artifact claim triggers this caveat. First version repeated the full ~200-word boilerplate once per flagged principle — 9 of 12 stable principles on object 35, ~1,800 words of near-duplicate text. Fixed: one shared explanatory paragraph, then a compact bullet list of flagged principles with their evidence. Cut the analysis from 28,961 to 19,482 characters.
2. **Wrong-signal number** — the first version stated "N of N runs agree" using `fingerprint_stability_map` (the noisy per-run Haiku hit-count) for that count. Confirmed this can flatly contradict the `stable` classification itself: Anatomical Correspondence showed "3 of 9 runs agree" in the caveat while classified `stable` (which requires majority agreement) — the same signal-disagreement problem the Step B restructure was built to eliminate, reappearing because the caveat reached for the wrong stored field. Fixed by dropping the per-principle run fraction entirely; the family count (`3 independently trained model families`) still displays, correctly sourced from the same run list Step B classified against, not from `stability_map`.

**Verified live** — analysis 29 on object 35 (analysis 28, the pre-fix verbose version, deleted by Alan). Not yet displayed anywhere distinctly in the UI beyond being part of the stored/rendered `pass2_text` markdown — no separate "flags" panel exists; it just reads as the last section of the analysis.
