...
**Second verification (2026-07-02):** also re-ran ingest on object 22 (`A326274-0`, fingerprinted 2026-06-25, predates the fix) using its stored original images. Created object **id 32** (left in place, not deleted). All 9 runs show `images_sent: 2`; all 6 Gemini/OpenAI runs describe both views (one run used a different heading style than "Image 2"/"View 2" but still covered the exterior in its text). Object 22 itself untouched — `ingest.ts` has no in-place update path, only insert.
